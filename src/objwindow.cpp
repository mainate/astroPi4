...
void objWindow::startThread()
{
    ...
    int err = -1;
    if (functIndiport != 0)
        err = pthread_create(&threadsWin, NULL, functIndiport, (void*) &mutexNcurses);
    if (err == 0)
        runThread = true;
...
configIndiWindow::configIndiWindow(int _l, int _c, int _y, int _x, char* _textTitle, int8_t _linkPlace, kindOfBtn_e _btnType, void* (_ptrFuncSystem(void*))): objWindow(_l, _c, _y, _x, _ptrFuncSystem), textWindow(_l, _c, _y, _x, _textTitle, NULL), buttonWindow(_l, _c, _y, _x, _textTitle, _linkPlace, _btnType, _ptrFuncSystem)
{
    ...
    //Display first page of available/configured profile
...
bool configIndiWindow::readProfilesList()
{
    sqlite3 *dbfile;
    int rc;
    ...
    int pageNumber = 0;
    actualptr = pagesList;
    char query[25] = "SELECT name FROM profile";

    const char *homedir;
    homedir = getenv("HOME");

    char fileNameDB[PATH_MAX];
    strncpy(fileNameDB, getenv("HOME"), PATH_MAX);
    strncat(fileNameDB,"/servermanager/profiles.db", PATH_MAX);

#ifndef LAPTOP
    strncpy(fileNameDB,"/root/servermanager/profiles.db", PATH_MAX);
#endif
    rc = sqlite3_open(fileNameDB, &dbfile);
    if( rc ) {
    ...
    if ( sqlite3_prepare(dbfile, query, -1, &statement, 0 ) == SQLITE_OK )
    {
        while (!( rc == SQLITE_DONE || rc == SQLITE_ERROR))
        {
            ...
            if ( rc == SQLITE_ROW )
            {
                strncpy(actualptr->page.groupDev, (char*)"Profile", 31);
                actualptr->page.pageNumber = pageNumber;
                pagesList->numPage = pageNumber;

                strncpy(actualptr->page.line[lineNumber].driverName, (char*)sqlite3_column_text(statement, 0), 31);

                // set the flag isSelected (From configuration)
                //if (configXML->isConfigured(actualptr->page.line[lineNumber].driverName))
                if (!strcmp("Simulators", actualptr->page.line[lineNumber].driverName))
...
bool configIndiWindow::readDriversList()
{
    ...
    while (pListGroup != NULL)
    {
        ...
        while (pListDevice != NULL)
        {
            ...
            // Get Driver and version from XML file
...
{
    int i;
    int lenLine = get_winContentInfo()->_maxx +1;
    // Check length line before print
    char deviceN[lenLine];
    strncpy(deviceN, deviceName, lenLine);

    int lenBlank = (lenLine - strlen(deviceName)) / 2;
    // Fillin begin with space to center the string
    if (strlen(deviceName) < lenLine)
    {
        memmove( deviceN + lenBlank, deviceN, strlen( deviceN ) + 1 );
        memset( deviceN, ' ', lenBlank);
    }

    // Fillin with right space the line in case have to display reserse attibute
    memset(deviceN +strlen(deviceN), ' ', lenLine - strlen(deviceN));
    deviceN[lenLine] = '\0';

    char spaceLine[lenLine];
    // Prepare full line fill with space to make bigger selections active profile
    memset(spaceLine, ' ', lenLine);
    spaceLine[lenLine] = '\0';

    pthread_mutex_lock(&mutexNcurses);
    if (active) wattron(get_winContentInfo(), A_REVERSE);
    // Move the cursor at begin of the correct line on windows screen
    //Draw a white line before
    wmove(get_winContentInfo(), ((line + 1) * 4) -1, 0);
    wprintw(get_winContentInfo(), "%s", spaceLine);
    //Draw the text line
    wmove(get_winContentInfo(), (line +1) * 4, 0);
    wprintw(get_winContentInfo(), "%s", deviceN);
    //Draw a white line after
    wmove(get_winContentInfo(), ((line + 1) * 4) +1, 0);
    wprintw(get_winContentInfo(), "%s", spaceLine);
...
{
    // X and Y are absolute position froem screen.
    ...
    case 3:
    case 4:
    case 5: lineSelected(0);
        break;
    case 7:
    case 8:
    case 9: lineSelected(1);
        break;
    case 11:
    case 12:
    case 13: lineSelected(2);
        break;
    case 15:
    case 16:
    case 17: lineSelected(3);
        break;
    case 19:
    case 20:
    case 21: lineSelected(4);
        break;
    case 23:
    case 24:
    case 25: lineSelected(5);
        break;
    case 27:
    case 28:
    case 29: lineSelected(6);
        break;
    }
    return CONTINUE;
}
